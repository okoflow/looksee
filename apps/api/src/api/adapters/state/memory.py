...
class InMemoryRuntimeState:
    ...
    def note_worker_retry(
        ...
        self._worker_retries[camera_id] = (revision, attempts + 1, now + timedelta(seconds=delay))
    ...
    def swap_track_point(
        ...
        self._track_points[key] = (now, point)
    ...
    def touch_event_cooldown(self, camera_id: UUID, run_id: UUID, kind: EventKind) -> None:
        now = datetime.now(UTC)
        self._event_cooldown[(camera_id, run_id, kind)] = now
